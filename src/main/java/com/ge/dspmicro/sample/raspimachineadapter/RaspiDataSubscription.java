 ...
 */
public class RaspiDataSubscription
        implements Runnable, IDataSubscription
{
    ...
    private Map<UUID, RaspiDataNode>       nodes         = new HashMap<UUID, RaspiDataNode>();
     ...
     */
    public RaspiDataSubscription(ISubscriptionMachineAdapter adapter, String subName, int updateInterval,
            List<RaspiDataNode> nodes)
    {
        if ( updateInterval > 0 )
        {
    ...
    public void run()
    {
        if ( !this.threadRunning.get() && this.nodes.size() > 0 )
        {
            ...
            while (this.threadRunning.get())
            {
                ...
                ((RaspiSubscriptionMachineAdapterImpl) this.adapter).putData(data);
